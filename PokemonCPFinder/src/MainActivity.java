...
public class MainActivity {
	...
	public static void main(String[] args) {
		...
		writer.println(
				"name,form,type1,type2,hp,atk,def,cp,level,hpIV,atkIV,defIV,quick,type,pwr,nrg,dur,charge,type,pwr,nrg,dur");
		...
		for (int n = 0; n < pokemonList.size(); n++) {
			...
			for (level = LEVEL_MAX * 2; level >= 2; level--) {
				...
				// System.out.println(curr.name + "," + curr.form + "," + cp + "," + (double)
				// level / 2 + "," + hpIV + "," + atkIV + "," + defIV);
				if (cp < 1500)
					break;
			}
			// writer.println("name,form,type1,type2,hp,atk,def,cp,level,hpIV,atkIV,defIV,quick,type,pwr,nrg,dur,charge,type,pwr,nrg,dur");
			writer.println(curr.toString() + "," + cp + "," + (double) level / 2 + "," + hpIV + "," + atkIV + ","
					+ defIV + "," + curr.quickMove.toString() + "," + curr.chargeMove.toString());
		}
		writer.close();
	}
		...
		try {
			...
			while (scan.hasNext()) {
				...
				pokemonList.add(new Pokemon(fields[0], fields[1], fields[2], fields[3], Integer.parseInt(fields[4]),
						...
						quickMoveList.get(Integer.parseInt(fields[7])),
						chargeMoveList.get(Integer.parseInt(fields[8]))));
			}
			scan.close();
		} catch (FileNotFoundException e) {
			// TODO Auto-generated catch block
